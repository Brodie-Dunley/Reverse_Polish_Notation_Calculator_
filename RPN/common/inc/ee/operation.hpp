...
public:
	...
	virtual void perform(std::stack<Operand::pointer_type>& operandStack) const = 0;
};

extern std::map<std::string, std::function<Integer::value_type(int,int)>> double_operations;
extern std::map<std::string, std::function<Integer::value_type(int)>>single_operations;
extern bool isFunction(const Token::pointer_type& token);
